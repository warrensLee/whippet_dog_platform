...
def test_add_dog_title_ok(admin_session, dog_factory):
    dog = dog_factory()
    with patch("backend.controller.dog_title.send_titles_email", return_value=True):
        with patch("backend.controller.dog_title.generate_title_pdf", return_value=True):
