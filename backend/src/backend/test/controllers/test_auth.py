...
from backend.classes.registration_invite import RegistrationInvite
import database
import pytest
from backend.classes.password_reset import PasswordResetToken
import secrets
...
def test_login_wrong_password(client):
    with patch("backend.controller.authentication.validate_turnstile", return_value=True):
...
def test_login_bad_turnstile(client):
    with patch("backend.controller.authentication.validate_turnstile", return_value=False):
        response = client.post("/api/auth/login", json={
            "username": "test@test.com",
            "password": "password",
            "cf_token": "whatever"
...
def test_forgot_password_no_token(client):
    with patch("backend.utils.email_service.send_reset_email", return_value=None):
        with patch("backend.controller.authentication.validate_turnstile", return_value=True):
...
def test_forgot_password_no_identifier(client):
    with patch("backend.controller.authentication.send_reset_email", return_value=None):
        with patch("backend.controller.authentication.validate_turnstile", return_value=True):
...
def test_invite_ok(admin_session): 
    with patch("backend.controller.authentication.send_invite_email"):
